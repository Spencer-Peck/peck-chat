...
function getConversationsFromDb(id, callback) {
	...
	sql += " INNER JOIN users other_users ON other_participants.user_id = other_users.id AND other_users.id = messages.user_id INNER JOIN participants auth_user ON auth_user.conversation_id = conversations.id";

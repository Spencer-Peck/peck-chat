...
function getConversationsFromDb(id, callback) {
	var sql = " SELECT distinct ON (conversations.id) conversation_id, messages.user_id AS last_message_owner_id, auth_user.messages_read_at < messages.created_at AS has_unread_messages, messages.id, messages.content, messages.created_at,";
